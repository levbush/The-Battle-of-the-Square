...
class UnitBase:
    type: int
    def __init__(self, owner: Player, pos: tuple[int, int], max_health: int, attack: int, defense: int, movement: int, range: int):
        self.owner = owner
        self.pos = pos
    ...
    def attack_unit(attacker, defender: 'UnitBase'):
        if attacker.owner == defender.owner:
            return
        if abs(attacker.pos[0] - defender.pos[0]) > attacker.range or abs(attacker.pos[1] - defender.pos[1]) > attacker.range:
            return
        # TODO: add particles
        ...
        if defender.health < 0:
            ...
            defender.die()
            return
        if abs(attacker.pos[0] - defender.pos[0]) <= defender.range and abs(attacker.pos[1] - defender.pos[1]) <= defender.range:
